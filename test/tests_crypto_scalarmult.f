...
! contains
  ...
  ! end subroutine collect_tests_crypto_scalarmult
  ...
  ! subroutine test_scalarmult(error)
    ...
    alice_pk = sodium_malloc(PARAM_crypto_scalarmult_BYTES)
    ! call sodium_malloc(alicepk, siz)
    bobpk = sodium_malloc(PARAM_crypto_scalarmult_BYTES)
    ! call sodium_malloc(bobpk, siz)
    ! call sodium_malloc(k, siz)
    k = sodium_malloc(PARAM_crypto_scalarmult_BYTES)
    ret = crypto_scalarmult_base(alicepk, alicesk)
    if(ret.ne.0) &
      error stop "error: crypto_scalarmult_base"
    ! call check(error, ret, 0)
    res = sodium_bin2hex(hex, hexlen, alicepk, PARAM_crypto_scalarmult_BYTES)
  ...
  ! end subroutine test_scalarmult

end module tests_crypto_scalarmult
