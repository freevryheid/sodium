module mod_crypto_onetimeauth
	...
	interface
		...
		endfunction crypto_onetimeauth_statebytes
		...
		&result(res)
			import::c_size_t
			integer(kind=c_size_t)::res
		endfunction crypto_onetimeauth_bytes
		...
		&result(res)
			import::c_size_t
			integer(kind=c_size_t)::res
		endfunction crypto_onetimeauth_keybytes

		function &
		&bind_crypto_onetimeauth_primitive()&
		...
		endfunction crypto_onetimeauth
		...
		&result(res)
			...
			integer(kind=c_long_long),value::inlen
			character(kind=c_char)::k
		endfunction crypto_onetimeauth_verify
		...
		endfunction crypto_onetimeauth_init
		...
		&result(res)
			...
			character(kind=c_char)::in
			integer(kind=c_long_long),value::inlen
		endfunction crypto_onetimeauth_update
		...
		&result(res)
			...
			type(crypto_onetimeauth_poly1305_state)::state
			character(kind=c_char)::out
		endfunction crypto_onetimeauth_final
